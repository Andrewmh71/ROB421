...
while True:
    ...
    if results.pose_landmarks:
        ...
        if i % 100 == 0:
            ...
            left_wrist[2] = left_wrist[2]-left_shoulder[2]
            left_elbow[2] = left_elbow[2]-left_shoulder[2]
            right_wrist[2] = right_wrist[2]-right_shoulder[2]
            right_elbow[2] = right_elbow[2]-right_shoulder[2]
            left_shoulder[2] = 0
            right_shoulder[2] = 0

            phiL = np.arctan2(left_elbow[1], left_elbow[0])
            phiR = np.arctan2(right_elbow[1], right_elbow[0])
            phieL = np.arctan2(left_wrist[1], left_wrist[0])
            phieR = np.arctan2(right_wrist[1], right_wrist[0])


            rL = np.sqrt(left_elbow[0]**2+left_elbow[1]**2+left_elbow[2]**2)
            phiL = np.arctan(left_elbow[1]/left_elbow[0])
            thetaL = np.arcsin(left_elbow[2]/rL)

            rR = np.sqrt(right_elbow[0]**2+right_elbow[1]**2+right_elbow[2]**2)
            phiR = np.arctan(right_elbow[1]/right_elbow[0])
            thetaR = np.arcsin(right_elbow[2]/rR)

            reL = np.sqrt(left_wrist[0]**2+left_wrist[1]**2+left_wrist[2]**2)
            phieL = np.arctan(left_wrist[1]/left_wrist[0])
            thetaeL = np.arcsin(left_wrist[2]/reL)

            reR = np.sqrt(right_wrist[0]**2+right_wrist[1]**2+right_wrist[2]**2)
            phieR = np.arctan(right_wrist[1]/right_wrist[0])
            thetaeR = np.arcsin(right_wrist[2]/reR)

            left_bicep_rotation = thetaeL
            right_bicep_rotation = thetaeR
            left_chest_angle_servo = thetaL
            right_chest_angle_servo = thetaR
            left_shoulder_angle_servo = phiL
            right_shoulder_angle_servo = phiR
            ...
            torso_tilt_angle_initial = np.arctan((abs(right_shoulder[1]-left_shoulder[1]))/(abs(right_shoulder[0]-left_shoulder[0])))
            ...
            torso_tilt_angle_initial = (torso_tilt_angle_initial + 2 * np.pi) % (2*np.pi)

            # Convert angles to servo positions
            # Torso Tilt (60 - 180 degrees)
            print(f"Torso Tilt Angle: {np.rad2deg(torso_tilt_angle_initial):.2f}°")
            if(left_shoulder[1]>right_shoulder[1]):
                torso_tilt_angle_initial = -torso_tilt_angle_initial
            torso_tilt_angle_servo = 115+np.rad2deg(torso_tilt_angle_initial)
            torso_tilt_angle_servo = np.clip(torso_tilt_angle_servo,80,160)
            ...
            print(f"Torso Tilt servo position: {torso_tilt_angle_servo}")
