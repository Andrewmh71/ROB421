...
# Environment
ground_plane = fcl.CollisionObject(fcl.Plane(np.array([0, 1, 0], dtype=np.float64), 0))        # Ground at Z = 0
wall_plane = fcl.CollisionObject(fcl.Plane(np.array([1, 0, 0], dtype=np.float64), 0.06))      # Wall at X = 60mm
body_plane = fcl.CollisionObject(fcl.Plane(np.array([0, 0, 1], dtype=np.float64), 0.2))       # Body at Y = 20mm
...
ground_mesh = trimesh.creation.box(extents=[wall_size, wall_thickness, wall_size])
ground_mesh.apply_translation([0, -base_z, 0.075])
ground_mesh.visual.face_colors = blue_transparent

# Wall plane, back of chair, (transparent red)
wall_mesh = trimesh.creation.box(extents=[wall_thickness, wall_size, wall_size])
wall_mesh.apply_translation([-0.06 + wall_thickness/2, -.25, -0.2+ wall_size/2])
wall_mesh.visual.face_colors = red_transparent

# Body plane (transparent green)
body_mesh = trimesh.creation.box(extents=[wall_size, wall_size, wall_thickness])
body_mesh.apply_translation([0, -0.2 - wall_thickness/2, wall_size/2-.02])
